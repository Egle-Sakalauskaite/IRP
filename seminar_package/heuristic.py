import numpy as np
from . import heuristics_helper as hh
...
def heuristic_main(FILE, SAFETY_TIME, SAFETY_QUANTITY, MINIMUM_DELIVERY_QUANTITY_PROPORTION):
    # initialization
    parameters = excel_reader.parameter_reader(FILE)
    time_periods = hh.periods_in_order(parameters["time windows"])
    consumption_forecast = parameters["forecast matrix minutes"]
    initial_inventory = hh.consume(parameters["tank initial quantity"], range(parameters["number customer"] + 2), consumption_forecast, 0, time_periods[0, 0])
    ...
    # for period_idx in range(8):
        ...
        urgency, total_demands = hh.ranking_urgency(parameters, initial_inventory.copy(), start_time)
        ...
        if len(filtered_customers) == 0:
            initial_inventory = hh.consume(initial_inventory, range(parameters["number customer"] + 2), consumption_forecast, start_time, end_time)
        ...
        while len(routed) < len(filtered_customers):
            ...
            minimal_delivery_quantity = hh.minimal_delivery_quantity(parameters, initial_inventory, total_demands, trailer, next_to_insert, SAFETY_QUANTITY)
            ...
            if state[cluster]["current tank level"] < minimal_delivery_quantity*MINIMUM_DELIVERY_QUANTITY_PROPORTION:
                ...
                try:
                    state[cluster] = hh.update_state(parameters, initial_trailer_tank[trailer], initial_inventory, customers_in_cluster, start_time, period_result_copied)
                    ...
                    state[cluster] = hh.update_state(parameters, initial_trailer_tank[trailer], initial_inventory, customers_in_cluster, start_time, period_result[cluster])
            ...
            for insert_position in range(len(period_result[cluster]["Operations"]) + 1):
                ...
                current_inventory = hh.consume(initial_inventory.copy(), next_to_insert, consumption_forecast, start_time, arrival_time - 1)
                delivery_quantity = hh.delivery_quantity(parameters, state[cluster], total_demands, current_inventory, next_to_insert, SAFETY_QUANTITY)
        ...
        initial_inventory = hh.consume(initial_inventory.copy(), range(parameters["number customer"] + 2), consumption_forecast, start_time, end_time)
    ...
    # print(solution)
    try:
        cost = hh.check_feasibility(solution, FILE)
...
objective, solution = heuristic_main(FILE, SAFETY_TIME, SAFETY_QUANTITY, MINIMUM_DELIVERY_QUANTITY_PROPORTION)
end_time = time.time()
...
# for i in range(0, 901, 300):
#     for j in range(0, 26, 5):
#         for k in range(5, 11):
#             k = k / 10
#             objective, solution = heuristic_main(FILE, i, j, k)
#             objectives.append({"configuration": (i, j, k), "objective value": objective})
...
JSON_output.write_to_JSON(solution, "heuristic_tiny_precise")
...
# JSON_output.total_cost(solution, parameters)